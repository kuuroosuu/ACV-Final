...
void main(int argc, char **argv){
  ...
  int t;
	...
	if (argc!=4){
	//	printf("argc=%d\n", argc);
		printf("Usage: m inputFeaFile outputFeaFile\n");
		exit(1);
	}
...
//	ifPtr=fopen("G:\\MVA\\debug\\test.mfc", "rb");
//	ofPtr=fopen("G:\\MVA\\debug\\test_mva.mfc", "wb");
...
//	copy HTK headers
  ...
  // need to swap for little-endian architecture
	byteSwap(&sampSize); byteSwap(&parmKind);
	byteSwap(&nSamples); byteSwap(&sampPeriod);
