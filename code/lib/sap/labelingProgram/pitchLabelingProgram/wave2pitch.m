function pitch=wave2pitch(au, ptOpt, showPlot);
% wave2pitch: pitch tracking from a given audio
%
%	Usage:
%		pitch=wave2pitch(au, ptOpt, showPlot); 
%			au: audio
...
%		showPlot=1;
%		wave2pitch(au, ptOpt, showPlot);

%	Roger Jang, 20021201, 20120409, 20150409

if nargin<1, selfdemo; return; end
if nargin<2 | isempty(ptOpt), ptOpt=ptOptSet(fs, nbits); end
...
for i=1:frameNum
	frame=frameMat(:, i);
	[pitch(i), acf, amdf, nsdf]=frame2pitch4labeling(frame, ptOpt);
end
...
if showPlot
	...
	targetPitch=pitch;
	if isfield(ptOpt, 'targetPitchFile') & exist(ptOpt.targetPitchFile)	% 正確音高所在的檔案
	...
	if length(targetPitch)<frameNum, fprintf('\tIgnore the pitch within %s.\n', ptOpt.targetPitchFile); targetPitch=pitch; end		% If the length is not right, ignore the pitch from the PV file.
	temp=targetPitch; temp(temp==0)=nan;
	targetPitchH=line(frameTime, temp, 'color', 'k', 'marker', '.');		% 正確音高
	title(titleStr);
	tempPitch=[pitch(:); targetPitch(:)]; tempPitch(tempPitch==0)=[];	% For computing pitch range
	...
	uicontrol('string', 'Play computed pitch', 'Callback', 'userDataGet; pvPlay(pitch, pitchRate);', 'position', [separation+1*(separation+width), 10, width, 20]);
	uicontrol('string', 'Play wave & computed pitch', 'Callback', 'userDataGet; pitchObj.signal=pitch; pitchObj.frameRate=pitchRate; wavePitchPlay(au, pitchObj);', 'position', [separation+2*(separation+width), 10, width, 20]);
	...
	uicontrol('string', 'Play wave & labeled pitch', 'Callback', 'userDataGet; pitchObj.signal=targetPitch; pitchObj.frameRate=pitchRate; wavePitchPlay(au, pitchObj);', 'position', [separation+4*(separation+width), 10, width, 20]);
...
%		buttonH=wavePitchPlayButton(au, pitchObj, pitchObj2);
%		set(buttonH(end), 'string', 'Play GT pitch')
%	else
%		buttonH=wavePitchPlayButton(au, pitchObj);
%	end
end
