function onset = odByPhaseDev(y,fs,nbits,odPrm,plotOpt,groundTruth)
% Perform onset detection (OD) for a given music signal
% odByPhaseDev: OD based on phase deviation only
%   Usage onset = odByPhaseDev(y,fs,odPrm,plotOpt,groundTruth)
...
%		odPrm=odPrmSet(fs);
%		plotOpt=1;
%		onset=odByPhaseDev(y, fs, nbits, odPrm, plotOpt);
% 
...
switch odPrm.phaseDev,
    ...
    case 1  % weighted phase deviation (WPD)
        ...
        phaseDev = sum( magSpecMat(:,3:end) .* abs(phaseSpecMat) ) / frameSize;
...
% (find local maxima)
cand = localMax(normalPhaseDev, odPrm.wSize, odPrm.mSize, odPrm.delta, odPrm.alpha);
